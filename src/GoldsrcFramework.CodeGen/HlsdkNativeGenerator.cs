...
internal static class HlsdkNativeGenerator
{
    ...
    static int Main(string[] args)
    {
        ...
        var humanoidTypeMappings = LoadMappings(Path.Combine(repo, "src", "GoldsrcFramework.CodeGen", "HumanoidTypeMapping.txt"));
        var humanoidVectorFields = LoadSet(Path.Combine(repo, "src", "GoldsrcFramework.CodeGen", "HumanoidVectorFields.txt"));
        var gen = new Generator(compilation, mappings, humanoidTypeMappings, humanoidVectorFields);
        var text = gen.Generate();
        Directory.CreateDirectory(Path.GetDirectoryName(output)!);
        WriteGeneratedFile(output, text);
        gen.GenerateHumanoidFiles(Path.GetFullPath(Path.Combine(Path.GetDirectoryName(output)!, "..")));
    ...
    sealed class Generator(CppCompilation compilation, Dictionary<string, string> mappings, Dictionary<string, string> humanoidTypeMappings, HashSet<string> humanoidVectorFields)
    {
        ...
        public void GenerateHumanoidFiles(string nativeDir)
        {
            Directory.CreateDirectory(nativeDir);
            foreach (var pair in humanoidTypeMappings.GroupBy(x => x.Value, StringComparer.OrdinalIgnoreCase).Select(g => g.First()))
            {
                ...
                EmitHumanoidClass(sb, cls, pair.Value, rawName, renameFields: true);
        ...
        void EmitHumanoidClass(StringBuilder sb, CppClass c, string humanoidName, string rawName, bool renameFields)
        {
            EmitHumanoidXmlDocumentation(sb, c, "");
            sb.AppendLine("[StructLayout(LayoutKind.Sequential)]").Append("public unsafe struct ").AppendLine(humanoidName).AppendLine("{");
            foreach (var f in c.Fields)
            {
                EmitHumanoidXmlDocumentation(sb, f, "    ");
                sb.Append("    public ").Append(HumanoidFieldType(rawName, f)).Append(' ').Append(Safe(renameFields ? HumanoidFieldName(f.Name) : f.Name)).AppendLine(";");
            }
            sb.AppendLine("}\n");
        }

        void EmitHumanoidXmlDocumentation(StringBuilder sb, CppElement element, string indent)
        {
        ...
        void EmitHumanoidCastHelpers(StringBuilder sb)
        {
            sb.AppendLine("public static unsafe class HlsdkHumanoidCast");
            sb.AppendLine("{");
            foreach (var pair in humanoidTypeMappings.GroupBy(x => x.Value, StringComparer.OrdinalIgnoreCase).Select(g => g.First()))
            {
                var raw = ResolveGeneratedRawTypeName(pair.Key);
                var humanoid = pair.Value;
                if (raw is null) continue;
                sb.Append("    public static ").Append(humanoid).Append("* AsHumanoid(").Append(raw).Append("* value) => (").Append(humanoid).AppendLine("*)value;");
                sb.Append("    public static ").Append(raw).Append("* AsRaw(").Append(humanoid).Append("* value) => (").Append(raw).AppendLine("*)value;");
            }
            sb.AppendLine("}");
        }

        string? ResolveGeneratedRawTypeName(string configuredRawName)
        {
            var cls = _classes.Values.FirstOrDefault(c => string.Equals(CsTypeName(c, false), configuredRawName, StringComparison.OrdinalIgnoreCase) || string.Equals(c.Name, configuredRawName, StringComparison.OrdinalIgnoreCase));
            return cls is null ? null : CsTypeName(cls, false);
        }


        string HumanoidFieldName(string name)
        {
        ...
        string HumanoidFieldType(string rawTypeName, CppField field)
        {
            if (IsConfiguredVectorField(rawTypeName, field) && IsFloatArray3(field.Type)) return "Vector3";
            if (Strip(field.Type) is CppArrayType array) return HumanoidArrayFieldType(array);
            return HumanoidCsTypeName(field.Type, true);
        }

        string HumanoidArrayFieldType(CppArrayType array)
        {
            var element = Strip(array.ElementType)!;
            if (element is CppArrayType nested) return $"InlineArray{array.Size}<" + HumanoidArrayFieldType(nested) + ">";
            if (element is CppPointerType pointer)
            {
                var pointerType = HumanoidCsTypeName(pointer, true);
                var wrapperName = PointerWrapperName(pointerType);
                return $"InlineArray{array.Size}<{wrapperName}>";
            }
            return $"InlineArray{array.Size}<" + HumanoidCsTypeName(element, false) + ">";
        }

        bool IsConfiguredVectorField(string rawTypeName, CppField field) => humanoidVectorFields.Contains(rawTypeName + "." + field.Name);
        ...
        string HumanoidCsTypeName(CppType type, bool field)
        {
            var rawName = CsTypeName(type, field);
            return humanoidTypeMappings.TryGetValue(rawName, out var mapped) ? mapped : rawName;
        }
