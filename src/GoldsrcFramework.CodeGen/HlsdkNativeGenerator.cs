...
internal static class HlsdkNativeGenerator
{
    ...
    static int Main(string[] args)
    {
        ...
        var humanizerFileMappings = LoadMappings(Path.Combine(repo, "src", "GoldsrcFramework.CodeGen", "HumanizerFileMapping.txt"));
        var humanizerFieldNameMappings = LoadMappings(Path.Combine(repo, "src", "GoldsrcFramework.CodeGen", "HumanizerFieldNameMapping.txt"));
        var humanizerVectorFields = LoadSet(Path.Combine(repo, "src", "GoldsrcFramework.CodeGen", "HumanizerVectorFields.txt"));
        var gen = new Generator(compilation, mappings, humanizerTypeMappings, humanizerFileMappings, humanizerFieldNameMappings, humanizerVectorFields);
    ...
    sealed class Generator(CppCompilation compilation, Dictionary<string, string> mappings, Dictionary<string, string> humanizerTypeMappings, Dictionary<string, string> humanizerFileMappings, Dictionary<string, string> humanizerFieldNameMappings, HashSet<string> humanizerVectorFields)
    {
        ...
        public void GenerateHumanizerFiles(string nativeDir)
        {
            ...
            {
                if (!TryGetHumanizerInfo(type, out var rawName, out var humanizedName, out var relativePath, out var reason, out var sourceHeader)) continue;
                var file = GetOrCreateHumanizerFile(files, relativePath);
                if (!file.Emitted.Add(humanizedName)) continue;
                EmitHumanizedType(file.Builder, type, humanizedName);
                fileMap.Add((rawName, humanizedName, relativePath, reason, sourceHeader));
            }

            if (_pointerWrappers.Count > 0)
            {
                const string pointerWrapperPath = "Core/PointerWrappers.cs";
                var file = GetOrCreateHumanizerFile(files, pointerWrapperPath);
                foreach (var wrapper in _pointerWrappers.OrderBy(x => x.Key))
                {
        ...
        (StringBuilder Builder, HashSet<string> Emitted) GetOrCreateHumanizerFile(Dictionary<string, (StringBuilder Builder, HashSet<string> Emitted)> files, string relativePath)
        {
            ...
            file = (new StringBuilder("// <auto-generated/>\nusing System;\nusing System.Runtime.InteropServices;\nusing GoldsrcFramework.LinearMath;\nusing NativeInterop;\n\nnamespace GoldsrcFramework.Engine.Native;\n\n"), new HashSet<string>(StringComparer.OrdinalIgnoreCase));
            files[relativePath] = file;
            return file;
        }

        bool TryGetHumanizerInfo(CppType type, out string rawName, out string humanizedName, out string relativePath, out string reason, out string sourceHeader)
        {
            type = Strip(type)!;
            sourceHeader = (type as CppElement)?.SourceFile ?? string.Empty;
            if (type is CppTypedef td && Strip(td.ElementType) is CppPrimitiveType)
            {
                if (!ShouldEmitHumanizedPrimitiveTypedef(td))
                {
                    rawName = humanizedName = relativePath = reason = string.Empty;
                    return false;
                }
                rawName = td.Name;
                humanizedName = humanizerTypeMappings.TryGetValue(rawName, out var mapped) ? mapped : rawName;
                relativePath = ResolveHumanizerRelativePath(rawName, null, humanizedName, sourceHeader, out reason);
                return true;
            }
            if (type is CppEnum e)
            {
                rawName = CsTypeName(e, false);
                humanizedName = humanizerTypeMappings.TryGetValue(rawName, out var mapped) ? mapped : rawName;
                relativePath = ResolveHumanizerRelativePath(rawName, e.Name, humanizedName, sourceHeader, out reason);
                return true;
            }
            if (type is CppClass c && c.IsDefinition)
            {
                if (HasUnsupportedAnonymousField(c) && !string.Equals(CsTypeName(c, false), "mstudioanimvalue_t", StringComparison.OrdinalIgnoreCase))
                {
                    rawName = humanizedName = relativePath = reason = string.Empty;
                    return false;
                }
                rawName = CsTypeName(c, false);
                humanizedName = humanizerTypeMappings.TryGetValue(rawName, out var mappedName)
                    ? mappedName
                    : humanizerTypeMappings.TryGetValue(c.Name, out mappedName)
                        ? mappedName
                        : rawName;
                relativePath = ResolveHumanizerRelativePath(rawName, c.Name, humanizedName, sourceHeader, out reason);
                return true;
            }
        ...
        string? HumanizerTypeKey(CppType type)
        {
            ...
            return type switch
            {
                ...
                CppClass c when c.IsDefinition => CsTypeName(c, false),
                CppEnum e => CsTypeName(e, false),
                _ => null,
            };
        }

        static bool IsAdditionalHumanizerHeader(string? sourceFile)
        {
            if (string.IsNullOrWhiteSpace(sourceFile)) return false;
            var normalized = sourceFile.Replace('\\', '/');
            return normalized.EndsWith("/engine/studio.h", StringComparison.OrdinalIgnoreCase)
                || normalized.EndsWith("/common/r_studioint.h", StringComparison.OrdinalIgnoreCase)
                || normalized.EndsWith("/common/studio_event.h", StringComparison.OrdinalIgnoreCase);
        }

        bool ShouldEmitHumanizedPrimitiveTypedef(CppTypedef typedef)
        {
            var name = typedef.Name;
            if (mappings.ContainsKey(name)) return false;
        ...
        {
            return c.Fields.Any(f => CsTypeNameStatic(Strip(f.Type)!).StartsWith("__Anonymous", StringComparison.Ordinal));
        }
        ...
        {
            if (humanizerFileMappings.TryGetValue(rawName, out var explicitPath) ||
                (nativeName is not null && humanizerFileMappings.TryGetValue(nativeName, out explicitPath)))
            {
        ...
        {
            type = Strip(type)!;
            if (type is CppTypedef td && Strip(td.ElementType) is CppPrimitiveType) { EmitHumanizedPrimitiveTypedef(sb, td, humanizedName); return; }
            if (type is CppEnum e) { EmitHumanizedEnum(sb, e, humanizedName); return; }
            if (type is CppClass c && c.IsDefinition) EmitHumanizedClass(sb, c, humanizedName, CsTypeName(c, false), renameFields: true);
        }

        static string NormalizeRelativePath(string path) => path.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);
        ...
        {
            if (string.Equals(rawName, "mstudioanimvalue_t", StringComparison.OrdinalIgnoreCase))
            {
                EmitMStudioAnimValue(sb, c, humanizedName);
                return;
            }
            ...
            foreach (var f in c.Fields)
            {
                ...
                sb.Append("    public ").Append(HumanizedFieldType(rawName, f)).Append(' ').Append(Safe(renameFields ? HumanizedFieldName(rawName, humanizedName, f.Name) : f.Name)).AppendLine(";");
            }
            sb.AppendLine("}\n");
        }
        ...
        void EmitHumanizedPrimitiveTypedef(StringBuilder sb, CppTypedef typedef, string humanizedName)
        {
            ...
            sb.AppendLine("[StructLayout(LayoutKind.Sequential)]")
              ...
              .Append(CsTypeName(typedef.ElementType, false)).AppendLine(" Value; }\n");
        }

        void EmitHumanizedEnum(StringBuilder sb, CppEnum e, string humanizedName)
        {
            EmitHumanizerXmlDocumentation(sb, e, "");
            sb.Append("public enum ").AppendLine(humanizedName).AppendLine("{");
        ...
        string HumanizedFieldName(string rawTypeName, string humanizedTypeName, string name)
        {
            if (humanizerFieldNameMappings.TryGetValue(humanizedTypeName + "." + name, out var mapped) ||
                humanizerFieldNameMappings.TryGetValue(rawTypeName + "." + name, out mapped))
                return mapped;
