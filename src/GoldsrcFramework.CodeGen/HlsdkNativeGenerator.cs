...
internal static class HlsdkNativeGenerator
{
    ...
    static int Main(string[] args)
    {
        ...
        var gen = new Generator(compilation, mappings);
        ...
        File.WriteAllText(inlineArrayOutput, CreateInlineArrayDefinitions(engineDir, gen.RequiredInlineArraySizes), Encoding.UTF8);

        var manifest = gen.CreateManifest(output, hlsdk);
        File.WriteAllText(Path.ChangeExtension(output, ".manifest.txt"), manifest, Encoding.UTF8);
    ...
    sealed class Generator(CppCompilation compilation, Dictionary<string, string> mappings)
    {
        ...
        {
            type = Strip(type)!;
            if (type is CppTypedef td && td.ElementType is CppPrimitiveType) { sb.AppendLine("[StructLayout(LayoutKind.Sequential)]").Append("public struct ").Append(td.Name).Append(" { public ").Append(CsTypeName(td.ElementType, false)).AppendLine(" Value; }\n"); return; }
            if (type is CppEnum e) { sb.Append("public enum ").AppendLine(CsTypeName(e, false)).AppendLine("{"); foreach (var item in e.Items) sb.Append("    ").Append(item.Name).Append(" = ").Append(item.Value).AppendLine(","); sb.AppendLine("}\n"); return; }
            ...
            foreach (var f in c.Fields) sb.Append("    public ").Append(FieldType(f.Type)).Append(' ').Append(Safe(f.Name)).AppendLine(";");
            sb.AppendLine("}\n");
        }
        ...
        string CsTypeName(CppType type, bool field)
        {
            ...
            return type is CppPrimitiveType ptype ? Primitive(ptype.Kind) : mappings.TryGetValue(displayName, out var dm) ? dm : displayName;
        }
